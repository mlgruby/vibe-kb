...
@given(
    content=st.text(
        min_size=1, max_size=200, alphabet=st.characters(whitelist_categories=("L", "N", "Zs"))
    ),
    hidden_dir=_hidden_dirname,
    filename=_safe_filename,
)
@settings(max_examples=150, suppress_health_check=[HealthCheck.function_scoped_fixture])
def test_search_wiki_never_returns_hidden_directory_files(tmp_path, content, hidden_dir, filename):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for r in results:
            assert not r["file"].startswith("."), f"Result from hidden dir leaked: {r['file']!r}"
            assert "/." not in r["file"], f"Result from hidden dir leaked: {r['file']!r}"
...
def test_search_wiki_never_returns_underscore_directory_files(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        results = search_wiki(wiki_dir, first_word)
        for r in results:
            assert not any(part.startswith("_") for part in Path(r["file"]).parts), (
